...
const RENDER_CACHE_VERSION = "cutout-deterministic-overlay-v1";
...
function buildStandeeOverlaySvg(
  ...
  // Scale: backdrop is ~200 cm spanning ~72 % of image height
  const pxPerCm = (imageH * 0.72) / 200;
  ...
  for (const item of items) {
    ...
    for (let q = 0; q < item.quantity; q++) {
      const prefix = presetAssetId ? `${presetAssetId} · ` : "";
      standees.push({ heightCm: item.heightCm, label: `${prefix}${item.heightCm}cm` });
    }
  }
  standees.sort((a, b) => b.heightCm - a.heightCm);

  const elems: string[] = [];
  const betweenGap = Math.round(imageW * 0.018);
  let curX = Math.round(imageW * 0.05);

  for (const s of standees) {
    const heightPx  = Math.round(s.heightCm * pxPerCm);
    const widthPx   = Math.max(32, Math.round(heightPx * 0.26));
    const cx        = curX + widthPx / 2;
    const topY      = floorY - heightPx;
    const cornerR   = Math.round(widthPx * 0.28);
    const baseRy    = Math.max(5, Math.round(widthPx * 0.16));
    const fs        = Math.max(10, Math.min(15, Math.round(widthPx * 0.52)));

    // Bail out if we'd clip off the right edge
    if (curX + widthPx > imageW - 8) break;

    elems.push(
      // White halo behind the body
      `<rect x="${(cx - widthPx / 2 - 3).toFixed(1)}" y="${(topY - 3).toFixed(1)}" ` +
        `width="${(widthPx + 6).toFixed(1)}" height="${(heightPx + 6).toFixed(1)}" ` +
        `rx="${cornerR + 3}" ry="${cornerR + 3}" fill="white" opacity="0.80"/>`,
      // Body — soft gray with dashed outline
      `<rect x="${(cx - widthPx / 2).toFixed(1)}" y="${topY.toFixed(1)}" ` +
        `width="${widthPx.toFixed(1)}" height="${heightPx.toFixed(1)}" ` +
        `rx="${cornerR}" ry="${cornerR}" ` +
        `fill="#EBEBEB" stroke="#AAAAAA" stroke-width="2" stroke-dasharray="8,4" opacity="0.92"/>`,
      // Base foot ellipse
      `<ellipse cx="${cx.toFixed(1)}" cy="${floorY.toFixed(1)}" ` +
        `rx="${(widthPx / 2 * 1.35).toFixed(1)}" ry="${baseRy.toFixed(1)}" ` +
        `fill="#DADADA" stroke="#AAAAAA" stroke-width="1.5" opacity="0.92"/>`,
      // Label beneath base
      `<text x="${cx.toFixed(1)}" y="${(floorY + baseRy + fs + 3).toFixed(1)}" ` +
        `text-anchor="middle" font-size="${fs}" font-family="sans-serif" ` +
        `font-weight="600" fill="#333333" opacity="0.90">${s.label}</text>`,
    );

    curX += widthPx + betweenGap;
  }
